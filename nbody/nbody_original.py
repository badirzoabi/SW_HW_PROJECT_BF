...
def advance(dt, n, bodies=SYSTEM, pairs=PAIRS):
    for i in range(n):
        for (((x1, y1, z1), v1, m1),
            ...
            dy = y1 - y2
            dz = z1 - z2
            # ORIGINAL hotspot: general power operator x ** (-1.5)
...
if __name__ == "__main__":
    ...
    if os.environ.get("NBODY_PROFILE"):
        # Heavy fixed workload so perf collects enough samples (~seconds).
        offset_momentum(BODIES[DEFAULT_REFERENCE])
    ...
    runner.metadata['description'] = "n-body benchmark (original)"
